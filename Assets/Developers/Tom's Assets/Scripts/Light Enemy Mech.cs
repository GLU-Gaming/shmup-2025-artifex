    ...
    void Update()
    {
        ...
        if (AttackCoolDown < 0)
        {
            if (ReturnTrigger == 0)
            {
                ...
                AttackCoolDown = -0.1f;
                transform.position = new Vector3(transform.position.x - Time.deltaTime * 20, Mathf.Lerp(transform.position.y, Player.transform.position.y, Time.deltaTime * 2f), transform.position.z);
            }
        }
